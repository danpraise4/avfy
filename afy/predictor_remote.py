...
class PredictorRemote:
    ...
    def _send_recv_async(self, method, args, critical):
        ...
        if meta_recv['name'] == 'predict':
            ...
            try:
                # Try with strict_map_key=False to handle potential key type issues
                result = msgpack.unpackb(data_recv, strict_map_key=False, raw=False)
            except Exception as e:
                self.log(f"msgpack unpack error: {e}")
                self.log(f"data_recv length: {len(data_recv) if data_recv else 'None'}")
                self.log(f"meta_recv: {meta_recv}")
                # Try alternative unpacking method
                try:
                    import msgpack_numpy
                    result = msgpack_numpy.unpackb(data_recv)
                except Exception as e2:
                    self.log(f"Alternative unpacking also failed: {e2}")
                    # If it's a buffer size issue, try to handle it gracefully
                    if "buffer is too small" in str(e2):
                        self.log("Attempting to handle buffer size issue...")
                        # Try to return a dummy result to continue execution
                        result = None
                    else:
                        raise e
